...
public class AddGroupDialog {
    ...
    public void finishLoading(List<GroupInfo> data, List<GroupMembersInfo> memberList){
        ...
        if((memberList == null|| memberList.size() == 0) && data.get(0).getMemberCount() == 0){
            Toast.makeText(context, "Group doesn't exist", Toast.LENGTH_SHORT).show();
            return;
        }
        dialog.show();
        ...
        if(memberList == null || memberList.size() == 0){
           ...
           // adapter= new AddGroupMemberListAdapter(ddata.subList(0,6),context,dialog);
            adapter= new AddGroupMemberListAdapter(ddata.subList(0,Math.min(groupInfo.getMemberCount() , 6)),context,dialog);
            ...
            adapter= new AddGroupMemberListAdapter(ddata.subList(0,Math.min(ddata.size() , 6)),context, dialog);
        ...
        id.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Toast.makeText(context, io.openim.android.ouicore.R.string.id_copied, Toast.LENGTH_SHORT).show();
            }
        });
